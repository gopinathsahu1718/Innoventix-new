...
const TechIcon = ({ tech, isHovered }: { tech: Tech; isHovered: boolean }) => {
  ...
  return (
    <div
      className={`w-12 h-12 sm:w-14 sm:h-14 transition-all duration-300 ${isHovered ? 'scale-110' : 'scale-100'}`}
      style={{ filter: isHovered ? `drop-shadow(0 0 15px ${tech.color}40)` : 'none' }}
    >
...
export default function Technologies() {
  ...
  return (
    ...
    >
      ...
      <div className="absolute -right-12 top-16 h-56 w-56 rounded-full bg-cyan-100/30 blur-3xl pointer-events-none" />
      <div className="absolute left-6 bottom-8 h-44 w-44 rounded-full bg-sky-100/30 blur-3xl pointer-events-none" />

      <div className="relative mx-auto max-w-[1280px] px-4 sm:px-6 lg:px-8">
        <div className="grid gap-10 lg:gap-12 lg:grid-cols-[0.95fr_1.05fr] items-start">

          {/* Left Column - Heading */}
          <div className="space-y-6 lg:space-y-8">
            <div
              className={`inline-flex items-center gap-2 rounded-full border border-sky-200/70 bg-sky-50/70 px-4 py-1.5 text-xs uppercase tracking-widest text-sky-700 shadow-sm backdrop-blur-sm transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
                }`}
            >
            ...
            <h2
              className={`font-display text-3xl sm:text-4xl lg:text-5xl font-semibold tracking-tight text-slate-900 transition-all duration-700 leading-tight ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-6'
                }`}
            >
            ...
            <p
              className={`text-base sm:text-lg text-slate-600 leading-relaxed transition-all duration-700 ${isVisible ? 'opacity-100 blur-0' : 'opacity-0 blur-sm'
                }`}
            >
          ...
          <div
            className={`relative overflow-hidden rounded-3xl border border-slate-200/70 bg-slate-50/90 p-6 sm:p-8 shadow-lg transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
              }`}
          >
            <div className="absolute inset-x-6 top-0 h-px bg-gradient-to-r from-cyan-400/60 via-sky-400/50 to-transparent" />

            <div className="space-y-6">
              {/* Two Feature Cards */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="rounded-2xl border border-slate-200/70 bg-white/90 p-5 sm:p-6 shadow-sm">
                  <p className="text-xs uppercase tracking-widest text-slate-500">Trusted stack</p>
                  <p className="mt-3 text-lg font-semibold text-slate-900 leading-tight">
                    Engineered for speed, security, and scale.
                  </p>
                </div>

                <div className="rounded-2xl border border-slate-200/70 bg-white/95 p-5 sm:p-6 shadow-sm">
                  <p className="text-xs uppercase tracking-widest text-slate-500">Futuristic polish</p>
                  <p className="mt-3 text-lg font-semibold text-slate-900 leading-tight">
                    Glass-inspired cards and ambient glow details.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <div
                    key={item.highlight}
                    className="rounded-2xl bg-white/80 border border-slate-100 p-4 text-center"
                  >
                    <p className="font-semibold text-slate-900 text-sm">{item.highlight}</p>
                    <p className="mt-1 text-[10px] sm:text-xs text-slate-400 leading-tight">{item.description}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="mt-12 sm:mt-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
          {technologies.map((tech, index) => (
            <button
              ...
              onClick={() => setHoveredTech(hoveredTech === tech.name ? null : tech.name)} // Support tap on mobile
              className={`group relative overflow-hidden rounded-3xl border bg-white p-5 sm:p-6 text-left transition-all duration-500 hover:-translate-y-1 hover:shadow-xl active:scale-[0.985] ${hoveredTech === tech.name
                  ? 'border-cyan-300 shadow-md scale-[1.02]'
            ...
            >
              <div className="flex items-start gap-4">
                <div className="flex h-14 w-14 sm:h-16 sm:w-16 flex-shrink-0 items-center justify-center rounded-2xl border border-slate-100 bg-slate-50 transition-all group-hover:bg-cyan-50">
                ...
                <div className="pt-1">
                  ...
                  <p className="text-xs sm:text-sm text-slate-500 mt-1 tracking-wide">Leading edge</p>
                </div>
              </div>

              {/* Subtle glow */}
              <div
                className={`absolute -right-6 -top-6 h-20 w-20 rounded-full bg-cyan-100/40 blur-2xl transition-opacity duration-500 pointer-events-none ${hoveredTech === tech.name ? 'opacity-80' : 'opacity-40'
                  }`}
              />
            </button>
          ))}
